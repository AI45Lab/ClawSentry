...
1. `skills-safety-bench`：优先启动，因为 case、manifest、Harbor/Codex runner 和历史 dry-run 证据最完整；先做 dry-run / 单 case / 单 RD raw-vs-protected，正式分数仍需单列 provider/CLI technical failure。
2. `skill-inject`：第二阶段做 static scan + Codex/Claude/Gemini 小样本，评估 skill injection runtime 防护与误报。
3. `MSB` + `agentdog-atbench`：第三阶段并行推进。MSB 先做 MCP tool metadata/pre_action/post_action adapter 与 12 attack type smoke；ATBench 先补 labeled manifest 做 offline replay。
