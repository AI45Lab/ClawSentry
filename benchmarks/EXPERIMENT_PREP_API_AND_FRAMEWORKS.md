...
| `user-ailab-202602-kimi-k2.5` | ailab本地配置 | ✓ | OpenAI-compatible | `http://s-20260204175507-cqflp.ailab-pj.pjh-service.org.cn/v1` | `kimi-k2.5` | `<not configured>` | 未提供 | 最近一轮 chat HTTP 200。 |
...
| ailab本地配置 | `http://s-20260204175507-cqflp.ailab-pj.pjh-service.org.cn/v1` | `<not configured>` | `kimi-k2.5` | 最近一轮 chat HTTP 200。 |
...
| Claude Code | 已安装，`2.1.119` | `claude auth status` 显示 `loggedIn: true`、`oauth_token` | `claude --model <model>` | 默认 Anthropic/Claude Code 协议；OpenAI-compatible endpoint 直连测试超时。 |
...
| Kimi CLI | 已安装，`1.41.0` | 存在 `~/.kimi/config.toml`；无明确 status 命令 | `kimi -m <model>` | 本轮 `kimi-k2.5` prompt 返回 502。 |
...
| Codex | `glm-5` | ailab本地配置候选 | ✗ 不可用 | Codex ChatGPT 账号返回：`glm-5` model is not supported。 |
| Codex | `MiniMax-2.7-w8a8` | ailab本地配置候选 | ✗ 不可用 | Codex ChatGPT 账号返回：`MiniMax-2.7-w8a8` model is not supported。 |
| Claude Code | `glm-5` + ailab 202603 base URL | ailab本地配置 | ✗ 未通过 | `claude --bare --model glm-5` 指向 OpenAI-compatible endpoint 后 90s timeout；协议不匹配风险高。 |
| Claude Code | `MiniMax-2.7-w8a8` + ailab 15002 base URL | ailab本地配置 | ✗ 未通过 | `claude --bare --model MiniMax-2.7-w8a8` 指向 OpenAI-compatible endpoint 后 90s timeout；协议不匹配风险高。 |
| Gemini CLI | `gemini-3-flash-preview` | Gemini CLI 登录/Google Cloud | ✗ 未通过 | 返回 403：`Gemini for Google Cloud API ... disabled`。 |
| Kimi CLI | `kimi-k2.5` | Kimi CLI 本地配置 | ✗ 未通过 | `kimi --print -m kimi-k2.5 ...` 返回 502。 |
| a3s-code | N/A | SDK/transport | 未做 CLI smoke | 非 CLI；已确认 Python 包 1.7.2 安装。 |
