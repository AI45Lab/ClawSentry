...
| 2026-05-11 | `api-connectivity` | N/A | MiniMax/GLM/Kimi/Gemini relay | Codex / Claude Code / Kimi CLI / Gemini CLI | N/A | 用户指定 CLI/API 搭配 readiness smoke | `python benchmarks/scripts/api_connectivity.py --chat --soft-fail --timeout 60 --target user-minimax-2.7-w8a8-codex-responses --target user-ailab-202603-glm-5-codex-responses --target user-minimax-2.7-w8a8-claude-messages --target user-ailab-202603-glm-5-claude-messages --target user-ailab-202602-kimi-k2.5 --target boyue-gemini-native-gemini-3-flash-preview` | `benchmarks/results/api-connectivity/2026-05-11-requested-cli-smoke.md` | 通过 / 未出 benchmark 分 | 后续正式实验默认使用这些配置：Codex+ailab Responses、Claude Code+ailab Messages、Kimi CLI+ailab Kimi with NO_PROXY、Gemini CLI+Boyue `GOOGLE_GEMINI_BASE_URL`。只证明 provider/CLI 可用，不代表安全评测分数。 |
...
- 2026-05-11 已确认 Codex 0.130 与 ailab MiniMax/GLM 的兼容路径是 `wire_api="responses"`，不再使用旧的 `wire_api="chat"` provider 假设。下一轮 `skills-safety-bench` 真实执行应先切到本次记录的默认 provider 矩阵，再复测单 case。
