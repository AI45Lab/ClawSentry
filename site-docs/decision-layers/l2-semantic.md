...
!!! note "FSPR 边界"
    First-Use Skill Package Review (FSPR) 不是普通 L2 runtime semantic review。FSPR 面向 skill package 本身，只输出 bounded evidence 和 admission recommendation；Gateway policy 负责派生 action / review tier，不直接修改 registry/list-state，也不改写已完成 decision。
